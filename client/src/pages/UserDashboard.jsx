...
function statusPill(status) {
  const map = {
    ...
    declined: "bg-rose-100 text-rose-900 dark:bg-rose-950/50 dark:text-rose-200",
  };
...
export default function UserDashboard() {
  ...
  const favoriteSeed = useMemo(() => dash?.favoriteIds, [dash?.favoriteIds]);
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const [d, props] = await Promise.all([fetchUserDashboard(), fetchProperties()]);
        if (!cancelled) {
          setDash(d);
          setProperties(props);
        }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="grid gap-8 lg:grid-cols-[240px_1fr]">
        <DashboardSidebar title="Renter hub" items={sidebarItems} />
        <div className="space-y-12">
          ...
          <section id="favorites">
            <div className="mb-4 flex items-end justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-slate-600 dark:text-slate-400">Homes you love — synced with your saves.</p>
              </div>
            </div>
            {favoriteProps.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center dark:border-slate-700 dark:bg-slate-900/50">
                <p className="text-sm text-slate-600 dark:text-slate-400">No favorites yet. Explore listings and tap the heart.</p>
              </div>
            ) : (
          ...
          <section id="saved">
            <h2 className="mb-4 font-display text-xl font-semibold text-slate-900 dark:text-white">Saved properties</h2>
            {savedProps.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-10 text-center dark:border-slate-700 dark:bg-slate-900/50">
                <p className="text-sm text-slate-600 dark:text-slate-400">Save listings to compare later.</p>
              </div>
            ) : (
          ...
          <section id="requests">
            <h2 className="mb-4 font-display text-xl font-semibold text-slate-900 dark:text-white">Rental requests</h2>
            <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white dark:border-slate-700/80 dark:bg-slate-900">
              <table className="min-w-full divide-y divide-slate-200 text-sm dark:divide-slate-800">
                <thead className="bg-slate-50 dark:bg-slate-800/80">
                  <tr>
                    <th className="px-4 py-3 text-left font-semibold text-slate-700 dark:text-slate-200">Listing</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {dash.rentalRequests.map((r) => (
                    <tr key={r.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                      ...
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{r.submittedAt}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold capitalize ${statusPill(r.status)}`}>
                          {r.status}
                        </span>
                      </td>
                    </tr>
